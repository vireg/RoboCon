...
//  }
  ...
  if(mode==1)                                      //See-Saw
  {
    ...
    Serial.println("Mode 1");
      ...
      {
        if(holdauto) {
          digitalWrite(autobot_holder,HIGH);
          setModeLED(autobot_holder);
        }
        else {
...
if(override) holo_drive( x , y , t , line);
...
void line_poll()
{
  line_v1 = analogRead(line_1);
  line_v2 = analogRead(line_2);
  line_v3 = analogRead(line_3);
  line_v4 = analogRead(line_4);
  line_v5 = analogRead(line_5);
  line_v6 = analogRead(line_6);
  
  line_v7 = analogRead(line_7);
  line_v8 = analogRead(line_8);
  line_v9 = analogRead(line_9);
  line_v10 = analogRead(line_10);
  line_v11 = analogRead(line_11);
  line_v12 = analogRead(line_12);
...
void line_action()
{
  if((line_v3>=th1 && line_v3<=th2) || (line_v9>=th1 && line_v9<=th2))
  {
...
//  double timeChange = (double)(now - lastTime);
  if((line_v1>=th1 && line_v1<=th2) || (line_v7 >= th1 && line_v7>=th2)) error=2;
  else if((line_v2>=th1 && line_v2<=th2) || (line_v8>=th1 && line_v8<=th2)) error=1;
  if((line_v6>=th1 && line_v6<=th2) || (line_v12>=th1 && line_v12<=th2)) error=-2;
  else if((line_v2>=th1 && line_v2<=th2) || (line_v11>=th1 && line_v11)) error=-1;
